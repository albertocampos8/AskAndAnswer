    ...
    public class CustomCode
    {
        ...
        public string registerLocation(string input)
        {
            try
            {
                ...
                using (xDB.OpenConnection())
                {
                    ...
                    {
                        string locID = int.Parse(cmd.Parameters["@" + DBK.ID].Value.ToString()).ToString();
                        //Regenerate the app key object
                        string appKey = "kvpl_" + DBK.SP.spGETKVPFULLADDRESS;
                        List<string> currList = (List<string>)HttpContext.Current.Application[appKey];
                        if (currList != null)
                        {
                            HttpContext.Current.Application[appKey] = null;
                        }
                        clsUtil helper = new clsUtil();
                        currList = helper.PutKVPInDictionary(appKey);  
        ...
        public string UpdatePartInventory(string input)
        {
            try
            {
                ...
                return s.UpdatePartInventory(input, m_dlim);
            } catch(Exception ex)
            {
                return (ex.Message + AAAK.vbCRLF + ex.StackTrace).Replace(AAAK.vbCRLF, DynControls.html_linebreak_string());
            } 
